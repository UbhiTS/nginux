...
/** Parse a comma/space-separated string into clean ISO-3166-1 alpha-2 codes. */
...
/** The full set of countries allowed under country lock, deduped and order-stable
 *  (home first): the home country, the additional allowed countries (travel), and
 *  a break-glass env override. NGINUX_ALLOWED_COUNTRIES lets a locked-out admin
 *  regain access without the UI (set it on the host, restart) - it only ever
 *  *adds* to the allowlist, never removes. */
export function activeAllowedCountries(): string[] {
  const s = getSettings();
  const seen = new Set<string>();
  const out: string[] = [];
  for (const code of [
    ...parseCountryCodes(s.homeCountry),
    ...parseCountryCodes(s.allowedCountries),
    ...parseCountryCodes(process.env.NGINUX_ALLOWED_COUNTRIES ?? ""),
  ]) {
...
export function geoipStatus(): GeoipStatus {
  ...
  const countries = activeAllowedCountries();
